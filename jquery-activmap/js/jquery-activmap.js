  ...
  $.fn.extend({
    activmap: function (settings) {
      ...
      var map = new google.maps.Map(
        document.getElementById("activmap-canvas"),
        {
          zoom: s.zoom,
          center: latlng,
          mapTypeId: mapTypeId,
      ...
      _init = function () {
        ...
        $.each(s.places, function (i, place) {
          ...
          if (place.img != undefined && place.img != "")
            mycontent +=
              '<div class="activmap-brand"><img src="' + place.img + '"></div>';
          mycontent +=
            '<div class="activmap-details"><h4 class="activmap-title">' +
            place.title +
            "</h4>";
          if (place.address != undefined && place.address != "")
            mycontent +=
              '<div class="activmap-address">' + place.address + "</div>";
          if (place.phone != undefined && place.phone != "")
            mycontent +=
              '<div class="activmap-phone">' + place.phone + "</div>";
          if (place.url != undefined && place.url != "")
            mycontent +=
              '<a href="' +
              place.url +
              '" target="_blank" class="activmap-url">' +
              place.url +
              "</a>";
          ...
          place.html =
            '<div class="activmap-place" id="activmap-place_' +
            i +
            '"><h3>' +
            place.title +
            "</h3><p>" +
        ...
        centerMarker = new google.maps.Marker({
          ...
          position: latlng,
          icon:
            s.center_icon != "" && s.center_icon != undefined
              ? s.center_icon
              : s.icon,
        });
        ...
        $('input[name="activmap_radius"]').on("change", function () {
          s.radius = $(this).val();
          $('input[name="marker_type[]"], select[name="marker_type[]"]').each(
            function () {
              _update_places_tag($(this), false);
            }
          );
        ...
        //Filter change event
        $('input[name="marker_type[]"], select[name="marker_type[]"]').on(
          "change",
          function () {
        ...
        $('input[name="marker_type[]"]:checked')
          .add($('select[name="marker_type[]"]').prop("selected", true))
          .each(function () {
            _update_places_tag($(this), false);
          });
      ...
      _paddedBounds = function (npad, spad, epad, wpad) {
        ...
        var SWworld = new google.maps.Point(
        ...
        var NEworld = new google.maps.Point(
          ...
          .each(function () {
            ...
            if (val != "") {
              ...
              if (i == 0) p_ids = $.merge([], ids[val]);
                ...
                if (s.request == "strict")
                  p_ids = $.arrayIntersect(ids[val], p_ids);
                if (s.request == "large") {
